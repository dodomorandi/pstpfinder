...
#define PS_PER_SAS 5
    ...
    for(vector<Group>::const_iterator i = groups.begin(); i < groups.end(); i++)
    {
          ...
          j++)
      {
        if(&j->group == &(*i))
        {
          if(not writtenHeader)
          {
            ...
            for(vector<const Residue*>::const_iterator k = i->getResidues()
                .begin(); k < i->getResidues().end(); k++)
            ...
            pocketDetailLog << setfill(' ') << setw(12) << left
                            << "ps max area";
      ...
      if(bestPocket != pockets.end())
      {
        ...
        const vector<const Residue*>& pocketResidues = bestPocket->group
            .getResidues();
        for(vector<const Residue*>::const_iterator k = pocketResidues.begin();
            k < pocketResidues.end(); k++)
          pocketLog << " " << (*k)->index;
