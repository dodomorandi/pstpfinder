...
namespace PstpFinder
{
  ...
  SasAnalysis::save(const std::string& filename)
  {
    ...
    fs::copy_file(fs::path(old_filename), fs::path(filename));
  ...
  SasAnalysis::OperationThread::OperationThread(SasAnalysis& parent)
  {
    ...
    if(parent.mode == SasAnalysis::MODE_SAVE)
      this->operationThread = thread(
          bind(&SasAnalysis::OperationThread::threadSave, ref(*this)));
    else
      this->operationThread = thread(
          bind(&SasAnalysis::OperationThread::threadOpen, ref(*this)));
  }
